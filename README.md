...
Д/з 10.5.1 (HW-03) от Skillfactory🎓. Веб-приложение на Джанго для отработки полученных знаний по работе с асинхронными задачами через Celery.

Переход на Celery задач (рассылка при появлении новой статьи по подписке и рассылка каждый понедельник в 8:00 статей за прошлую неделю по подписке), что были выполнены в задании 9.5.4 (HW-03).
В качестве Redis запускаем локальный Redis-x64-3.0.504, точнее redis-server от имени админа (он в архиве в корне).